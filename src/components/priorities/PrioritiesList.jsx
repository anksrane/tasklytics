...
import { InputSearch, ButtonWithIcon, Loader, AddEditPriorities, DeletePriorities } from '../index.js';
import { getPrioritiesWithFilter } from '../../firebase/priorityServices/prioritiesService.js';
...
function PrioritiesList() {
  const [priorityData,setPrioritiesData] = useState([]);
  ...
  const [showAddEditPriorities, setShowAddEditPriorities] = useState(false);
  const [editingPrioritiesData, setEditingPrioritiesData] = useState(null);

  const [deletePriorities,setDeletePriorities] = useState(null);  
  const [showDeletePriorities, setShowDeletePriorities] = useState(false);

  // Function to handle edit click
  const handleOpenEditPopup = (priorities) => {
    setEditingPrioritiesData(priorities);
    setShowAddEditPriorities(true);
  };

  const handleCloseEditPopup = () => {
    setEditingPrioritiesData(null);
    setShowAddEditPriorities(false);
  };  

  // Function to Handle Delete Click
  const handleOpenDeletePopup = (priorities)=> {
    setDeletePriorities(priorities);
    setShowDeletePriorities(true);
  }

  const handleCloseDeletePopup = () => {
    setDeletePriorities(null);
    setShowDeletePriorities(false);
  }
  ...
  const fetchPriorities = useCallback(async (targetPage = 0) => {
    setLoading(true);

    const response = await getPrioritiesWithFilter({
      page: targetPage + 1,
      pageSize,
  ...
  useEffect(() => {
    fetchPriorities(currentPage); // fetch first page on mount
  ...
  const goToPage = (pageNumber) => {
    ...
    fetchPriorities(pageNumber);        // fetch new data
  };  
  ...
  return (
    <div className="mx-auto p-4 z-10">
      ...
      {showAddEditPriorities && (
        <AddEditPriorities
          onClose={handleCloseEditPopup}
          editingMode={!!editingPrioritiesData}
          priorityData={editingPrioritiesData}
          onPriorityAdded={() => {
            fetchPriorities(currentPage);
          }}
        />
      )}   

      {
        showDeletePriorities && (
          <DeletePriorities 
            onClose={handleCloseDeletePopup}
            priorityData={deletePriorities}
            onPriorityDeleted={() => {
              fetchPriorities(currentPage);
            }}
          />
