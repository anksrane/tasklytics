          ...
          <div className='bg-white w-auto min-w-96 h-fit flex flex-col p-4 cursor-auto' onClick={(e) => e.stopPropagation()}  >
            <IoIosWarning className='text-5xl m-auto mb-4 text-danger' />
            {
              ...
              :(
                  tasks.length>0 ? (
                    <>
                      <p className={`text-center mb-1 text-dark`}>This phase cannot be deleted because there are <strong>{tasks.length}</strong> active tasks linked to <strong>{phaseData.label}</strong>.</p>
                    ...
                    <>
                      <p className={`text-center mb-1 text-dark`}>Are you sure you want to delete <strong>{phaseData.label}</strong>?</p>
